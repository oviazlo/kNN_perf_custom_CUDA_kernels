...
struct New2KnnOpFunctor<CPUDevice, dummy> {
...
class New2KnnOp : public OpKernel {
public:
    explicit New2KnnOp(OpKernelConstruction *context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("New2Knn").Device(DEVICE_CPU), New2KnnOp<CPUDevice>);

#ifdef GOOGLE_CUDA
extern template struct New2KnnOpFunctor<GPUDevice, int>;
REGISTER_KERNEL_BUILDER(Name("New2Knn").Device(DEVICE_GPU), New2KnnOp<GPUDevice>);
#endif  // GOOGLE_CUDA
